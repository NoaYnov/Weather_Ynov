import requests
import json

link_temp ="https://public-api.meteofrance.fr/public/DPClim/v1/liste-stations/horaire?id-departement=13&parametre=temperature"
link_commande = "https://public-api.meteofrance.fr/public/DPClim/v1/commande-station/horaire?id-station=13001003&date-deb-periode=2024-01-01T14%3A00%3A00Z&date-fin-periode=2025-01-01T14%3A00%3A00Z"

token = "..BTPyuw64bNEoB78Ykwbeeaw2_7d8rj5r29I1Qfz8OzOVt2WM81MT-tpM8pNnQht-NC0aJwdOgqR-3BPM3gvnpRX4IOCckl2txBbFYMh87wyyXzdERL7bXtum7K4EmIhl75LcqrTOkK6orfoY9tkNg1ei3vVrxEeJd-A-OrTR6Xp-dqzNYx6alDm5mf_wU4qWQLGc2XQzzJgaOmiLhkePaIWpYJPA_hnJ_QgSVJIseAzIlWgdPgij-Y0D9flMj2MCncj_zcBn728XR_eDr5O8UHTWY-CGvNs19VLpqxtlxYH6jCRnDfnRtbhluFaQTCkN6rDjJiRGoBTACAEz7qNnaA"


def fetch_api_commande():
    headers = {
        'Authorization': 'Bearer ' + token,
        'Content-Type': 'application/json'
    }

    response = requests.get(link_commande, headers=headers)

    if response.status_code == 200:
        print("API is reachable.")
        print("Response JSON:")
        return response
    else:
        print(f"API returned an error: {response.status_code} - {response.text}")
    


def write_json_to_file(data, filename):
    with open(filename, 'w') as f:
        json.dump(data, f, indent=4)
    print(f"Data written to {filename}")





def main():
    write_json_to_file(fetch_api_commande(), "files/temperatures/13.csv")




if __name__ == "__main__":
    main()